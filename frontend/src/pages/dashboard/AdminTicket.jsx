...
const AdminTicket = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      <AdminSideNavigation activeSection="tickets" setActiveSection={() => {}} />
      
      <div className="flex-1 max-w-7xl mx-auto p-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Admin Ticket Dashboard</h1>
      ...
      {expanded && (
        <div className="border-t p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Info label="Location" value={formatLocationCategory(ticket.locationCategory)} />
            <Info label="Building" value={ticket.buildingName} />
            <Info label="Floor" value={ticket.floorNumber ? `Floor ${ticket.floorNumber}` : '-'} />
            <Info label="Room" value={ticket.roomNumber || '-'} />
            <Info label="Contact" value={ticket.contactPhone || '-'} />
            <Info label="Created" value={ticket.createdAt ? new Date(ticket.createdAt).toLocaleString() : '-'} />
            <Info label="Expected Completion" value={ticket.expectedCompletionAt ? new Date(ticket.expectedCompletionAt).toLocaleString() : '-'} />
          </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {ticket.attachments.map((attachment, idx) => (
                  <img
                    key={idx}
                    src={getImageSrc(attachment.fileUrl)}
                    alt={attachment.fileName || `Attachment ${idx + 1}`}
                    className="w-20 h-20 object-cover rounded border cursor-pointer hover:opacity-80"
                    onClick={() => window.open(getImageSrc(attachment.fileUrl), '_blank')}
                  />
                ))}
              </div>
            </div>
